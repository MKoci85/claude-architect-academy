...
- **Model IDs in new code examples**: match the generation already used across the site (`claude-opus-5`, `claude-sonnet-5`, `claude-haiku-4-5-20251001`, and `claude-fable-5-1` for the top-tier/most-capable-model case) rather than introducing a different generation in a single file. Never use a retired model ID (anything in the `3.x` family) or present an outdated-but-still-active one (e.g. `claude-opus-4-8`, `claude-sonnet-4-6`) as "the current model" — check `shared/models.md` via the `claude-api` skill if unsure what's current. An older ID is fine when a piece of content is deliberately about legacy-version behavior or model-retirement handling (e.g. a CCDV-F question testing SDLC maintenance-phase practice around retiring `claude-sonnet-4-6`) — don't blindly swap those.
...
`examen_cca_f_en.json` holds 265 domain-grouped questions (good conceptual bank), including multiple-response items (`correctAnswers` array + `selectCount`) alongside single-answer ones (`correctAnswer`). Multiple-response questions always have **5 options** (2 correct, 3 wrong) rather than the 4 used by single-answer questions — the 5th option exists specifically to keep random-guess odds low (1/10 vs. 1/6 with only 4) and to make room for an extra distractor. Each exam attempt guarantees a minimum **30% multiple-response floor per domain** (this is a pedagogical choice made for this simulator, not a documented feature of the real exam's item mix — see `MIN_MULTI_RATIO` in `public/practice/index.html`; the bank itself is authored to ~30-32% multi per domain so the floor doesn't force the same handful of questions into every attempt). The simulator samples by domain weight, not by the real exam's 4-of-6 scenario draw — see "Real exam format" above.

When authoring multiple-response questions, make the hardest of the 3 wrong options a **"half-right" trap**: one that gets one of the two correct answers right (or reuses correct terminology/mechanism from the question) but fails the other half in a subtle way — e.g. correctly names the right technique but misapplies it, or correctly identifies an action but omits a required detail (see existing questions like `D1-Q19`, `D5-Q04` for the pattern). This is harder to rule out than a distractor that's wrong on both counts, since it forces the test-taker to verify each half of the answer independently rather than pattern-matching the whole option as right or wrong.
