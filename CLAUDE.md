...
Content is loaded in two tiers so the app never has to download every collection's full body just to render the home page or run a search:
...
public/data/{lang}/*.json            ← one JSON file per collection/sub-collection, full body incl. blocks
         ↓ (build-index.mjs strips blocks, keeps flattened plain text for search)
public/data/{lang}/index-lite.json   ← generated, gitignored: collections → sections → articles,
                                        each article has id/title/summary/sourceFile + subsections
                                        with precomputed `text` (no blocks)
         ↓ (fetched at startup, `lang` from useLanguage() / localStorage 'wiki-lang')
src/searchEngine.js                  ← builds flat article index from index-lite.json, scores queries
         ↓
src/hooks/useWikiData.js             ← owns collections/search/navigation state, calls searchEngine + fetchLocalized
         ↓
src/App.jsx                          ← thin shell: composes useTheme/useLanguage/useWikiData, renders top-level views
...
`index-lite.json` is a **build artifact, not authored content** — it's generated by `scripts/build-index.mjs`, gitignored, and regenerated automatically by the `dataIndexPlugin` in `vite.config.js` every time `npm run dev` starts (and again on any `public/data/**/*.json` change while it's running) or `npm run build` runs. Never edit it by hand; edit the real per-collection file and `content.json` instead — the "Adding content" workflow below is unaffected.

`fetchLocalized(file, lang)` in `src/data/fetchLocalized.js` does the `/data/${lang}/${file}` fetch and returns `null` on a non-OK response — used both by `useWikiData`'s initial `index-lite.json` load and by `openArticle`'s on-demand full-article fetch. A `null` collection/item is filtered out, which is what makes untranslated content disappear in English mode instead of erroring.
...
| `vite.config.js` | Includes `practiceRoutePlugin` (serves `/practice`) and `dataIndexPlugin` (regenerates `index-lite.json`) |
| `scripts/build-index.mjs` | Generates `public/data/{lang}/index-lite.json` from `content.json` + the full collection files |
