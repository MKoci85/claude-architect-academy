...
`fetchLocalized(file, lang)` in `App.jsx` does the `/data/${lang}/${file}` fetch and returns `null` on a non-OK response — used both by the initial `index-lite.json` load and by `openArticle`'s on-demand full-article fetch. A `null` collection/item is filtered out, which is what makes untranslated content disappear in English mode instead of erroring.
